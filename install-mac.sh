...
if grep -q '"magicDNS"[[:space:]]*:[[:space:]]*false' <<<"$DNS_PREFS"; then
...
TS_VER=$("$TS_BIN" version 2>/dev/null | sed -n '1p' | tr -dc '0-9.')
TS_MAJOR=${TS_VER%%.*}
TS_REST=${TS_VER#*.}
TS_MINOR=${TS_REST%%.*}
...
GUEST_LIST=$(container list --all 2>/dev/null) || GUEST_LIST=""
if grep -q "[[:space:]]${GUEST_NAME}\$\|^${GUEST_NAME}[[:space:]]" <<<"$GUEST_LIST"; then
...
# ⚠️ `if [[ -x ]] && cmd | grep -qi`: under pipefail a successful match can be
# reported as failure (grep -qi exits on the match, the producer takes EPIPE).
# Capture first, then match a here-string. The `if` (not `&&`) keeps a
# non-existent $FW from aborting the script under `set -e`.
FW_STATE=""
if [[ -x "$FW" ]]; then FW_STATE=$("$FW" --getglobalstate 2>/dev/null) || FW_STATE=""; fi
if [[ -n "$FW_STATE" ]] && grep -qi "enabled" <<<"$FW_STATE"; then
