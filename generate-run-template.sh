...
generate_run_template() {
    ...
    # The `tailscale serve` proxy target — NOT always the primary port. See the
    # long note in parse-service-config.sh: the MCP pod's OAuth plane is a second,
    # loopback-only listener and :443 must land on THAT one (#831). Empty (every
    # other pod) means the primary port, so their run.sh is byte-identical.
    local serve_proxy_port
    serve_proxy_port=$(jq -r 'if has("serve_proxy_port") and .serve_proxy_port != null then (.serve_proxy_port|tostring) else "" end' <<<"$service_info")
    [[ -n "$serve_proxy_port" ]] || serve_proxy_port="$primary_port"
...
EOF
  ...
  "Web": {
    "\${TS_CERT_DOMAIN}:443": {
      "Handlers": {"/": {"Proxy": "http://127.0.0.1:$serve_proxy_port"}}
    }
  }
}
SERVEEOF
