...
generate_all_scripts() {
    ...
    # run.sh inlines the pod's `-e KEY=value` environment, which can hold
    # secrets (gateway secret, API keys), so the generated scripts must never
    # be world/group readable (S3). Generate them with a tight umask so they're
    # born 0600 — closing the window where a fresh file would briefly sit at
    # 0644 — and rm any prior copy first so a re-render can't inherit the old
    # world-readable mode through an in-place truncate. Restore the umask, then
    # add the owner exec bit (0700). They're only ever invoked as `sh ./run.sh`
    # by the controller (root), so dropping other/group access costs nothing.
    local old_umask
    old_umask=$(umask)
    umask 077
    rm -f "$service_dir"/*.sh 2>/dev/null || true
