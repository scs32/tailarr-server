...
parse_service_config() {
    ...
    # THE SERVE PROXY TARGET, WHICH IS NOT ALWAYS THE PRIMARY PORT (#831).
    # `tailscale serve` terminates TLS on :443 and proxies to exactly ONE
    # loopback port inside the pod's netns. For every ordinary pod that is the
    # primary port. The MCP pod is the exception that made this its own field:
    # it binds TWO listeners in one container —
    #   0.0.0.0:MCP_PORT        the raw tailnet whois plane (`/mcp` only, no
    #                           OAuth routes). This IS the primary port and must
    #                           STAY the primary port: it is what the tailnet
    #                           grant opens, what the readiness probe can reach
    #                           and the only one a tailnet peer can dial, so the
    #                           printed URL naming it is true.
    #   127.0.0.1:MCP_FUNNEL_PORT  the OAuth plane (/register, /authorize,
    #                           /token, /revoke, bearer `/mcp`). Loopback-only,
    #                           so serve/funnel is the ONLY way in.
    # Proxying :443 at the primary port there 404s every OAuth route, which is
    # exactly the shipped defect: no assistant could refresh a token.
    #
    # Empty (the overwhelmingly common case) means "the primary port", so every
    # other pod renders byte-identically to before. A PRESENT-BUT-MALFORMED
    # value is a HARD failure, never a silent fallback: falling back would
    # re-point the PUBLIC surface at a plane the operator did not choose, which
    # is the very defect this field exists to remove.
    #
    # ⚠️ AN ABSENT OVERRIDE IS PERSISTED AS EMPTY, NOT AS THE PRIMARY PORT, AND
    # THAT IS THE WHOLE POINT. Defaulting it here would write a COPY of
    # primary_port into every ordinary pod's .config.json at first render — and
    # a copy is a lie waiting to happen. Change a pod's `ports` and re-render:
    # primary_port follows, the stored copy does not, and the stale copy WINS,
    # because the renderer prefers the explicit field. That is a value that used
    # to be DERIVED becoming TRANSCRIBED, with the transcription outranking the
    # truth — the same shape as the defect this field exists to fix. The
    # fallback therefore lives in exactly ONE place: the renderer
    # (generate-run-template.sh), which reads primary_port when this is empty.
    local serve_proxy_port
    serve_proxy_port=$(jq -r 'if has("serve_proxy_port") and .serve_proxy_port != null then (.serve_proxy_port|tostring) else "" end' <<<"$config_json")
    if [[ -n "$serve_proxy_port" ]]; then
        if ! [[ "$serve_proxy_port" =~ ^[0-9]+$ ]] \
           || (( serve_proxy_port < 1 || serve_proxy_port > 65535 )); then
            log_error "serve_proxy_port must be a port number 1-65535 (got '$serve_proxy_port')"
            return 1
        fi
    fi
    ...
    service_info=$(jq -n \
        ...
        --arg serve_proxy_port "$serve_proxy_port" \
        ...
        '{
            ...
            serve_proxy_port: $serve_proxy_port,
