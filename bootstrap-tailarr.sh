...
controller_image_ref() {
    ...
    ref=$(podman inspect tailarr --format '{{.ImageName}}' 2>/dev/null | sed -n '1p') \
        || ref=""
    if [[ -z "$ref" ]]; then
        ...
        ref=$(podman inspect tailarr --format '{{.Config.Image}}' 2>/dev/null | sed -n '1p') \
            || ref=""
    fi
...
# This line lives inside the /root/start-pods.sh heredoc. That script is
# #!/bin/sh and sets no options at all, so a producer killed by SIGPIPE cannot
# become the pipeline's status there (and `<<<` is a bashism unavailable to it).
# pipefail-safe: inside the start-pods.sh heredoc, which is #!/bin/sh with no pipefail.
  ...
  if command -v systemctl >/dev/null 2>&1; then
    for unit_path in /etc/systemd/system/srv-*.mount; do
      ...
      if ! systemctl is-active --quiet "$unit"; then
        ...
        mp=$(sed -n 's/^Where=//p' "$unit_path" | sed -n '1p')
...
sleep "${WAIT:-10}"
running_names=$(podman ps --format '{{.Names}}') || running_names=""
if ! grep -q '^tailscale-tailarr$' <<<"$running_names"; then
...
FQDN=$(podman exec tailscale-tailarr tailscale status --json --peers=false 2>/dev/null \
    | grep -o '"DNSName": *"[^"]*"' | sed -n '1p' | cut -d'"' -f4 || true)
echo ""
...
if [ -f "$ADMIN_TOKEN_FILE" ]; then
    admin_tok=$(grep -o 'tailarr-admin-[A-Za-z0-9_-]*' "$ADMIN_TOKEN_FILE" | sed -n '1p' || true)
    if [ -n "$admin_tok" ]; then
        echo ""
